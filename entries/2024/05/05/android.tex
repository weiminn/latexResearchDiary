...
\subsection{Implement Zicheng Code inside AOSP13 Emulator}

Insert the following code at the bottom of \path{build/make/target/board/emulator_x86_64/device.mk}: 
\begin{lstlisting}
DISABLE_ARTIFACT_PATH_REQUIREMENTS := true
PRODUCT_PACKAGES += MINIMAManager
\end{lstlisting}
And paste in the \path{MINIMAManager.apk} inside \path{packages/apps/MINIMAManager} directory.

But the app installed via \path{adb install success_apps/60_com.atomic.apps.medical.drug.dictionary.apk} doesn't launch after preloading the MINIMAManager APK. If I remove the preload, then the app launches.
