% mainfile: ../../../../master.tex
\subsection{Connect Daemon to ART}
...
Have to always sync the changes between \path{aosp13/libnativehelper/include_jni/jni.h} and \path{aosp13/system/extras/module_ndk_libs/libnativehelper/include_jni/jni.h}, so insert the following function definition after the definition of VM Initialization functions: 
...
Add \path{k_WEI_TestFunc} into \path{MethodIndex} \texttt{enum} of \path{aosp13/libnativehelper/libnativehelper_lazy.c}.

Add following statement inside the \texttt{InitializeOnce} of \path{aosp13/libnativehelper/libnativehelper_lazy.c} after the definition of \texttt{BIND\_SYMBOL}: 
\begin{lstlisting}[language=C++]
// ADDED BY WEI MINN
BIND_SYMBOL(WEI_TestFunc);
\end{lstlisting}

Add following statement at the bottom of \path{aosp13/libnativehelper/libnativehelper_lazy.c}: 
\begin{lstlisting}[language=C++]
// ADDED BY WEI MINN
...
Here is the source code of the daemon in \path{aosp13/device/generic/goldfish/app/wei_daemon/weiminn.cpp}: 
\begin{lstlisting}[language=C++]
#include <unistd.h> 
#include <stdio.h> 
...
Add following statement at the bottom of \texttt{art} namespace inside \path{aosp13/art/runtime/jni/java_vm_ext.cc}: 
\begin{lstlisting}[language=C++]
// ADDED BY WEI MINN
...
Add following statement in \texttt{JniInvocationInit} function inside \path{aosp13/libnativehelper/JniInvocation.c}: 
\begin{lstlisting}[language=C++]
// ORIGINAL
...
Add following statement at the bottom of \texttt{JniInvocationImpl} \texttt{struct} in \path{aosp13/libnativehelper/JniInvocation.c}: 
\begin{lstlisting}[language=C++]
...
Change all the function return signatures to \texttt{const void*}, because of \texttt{extern "C"} in \path{java_vm_ext.cc}.
